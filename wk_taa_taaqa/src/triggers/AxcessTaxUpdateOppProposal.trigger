trigger AxcessTaxUpdateOppProposal on Account (after update) {
    ...
    Product2 Pr = [select id from product2 where name = 'CCH Axcess Tax - Tax Update - MTS'];
    Apttus_Config2__PriceListItem__C PLIID = [select Id from Apttus_Config2__PriceListItem__C where Apttus_Config2__ProductId__c = :Pr.Id];
    PriceBookEntry PBE = [select Id from pricebookentry where productCode = '220407'];
          ...
          ,erp_account__c = A.Tax_Update_Account_number__c );            
            ...
            Insert C;
             ...
             Apttus_Config2__LineItem__c CLI = new Apttus_Config2__LineItem__c(
                ...
                Apttus_Config2__ProductId__c = Pr.Id,
              ...
              apttus_proposal__proposal__c = p.Id,
                apttus_proposal__product__c = Pr.Id,
                ...
                apttus_qpconfig__PriceListItemId__c = PLIID.Id);
                insert PLI;
