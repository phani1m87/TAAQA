trigger HasAnswerConnect on Apttus_Proposal__Proposal_Line_Item__c (after insert, after update) {
    ...
    parentProposal = new Map<Id, Apttus_Proposal__Proposal__c>([SELECT id,Has_Answer_Connect__c,(Select id from R00N70000001yUfBEAU__r where product_family__c= 'AnswerConnect') FROM Apttus_Proposal__Proposal__c WHERE ID IN :listIds]);
    ...
    for (Apttus_Proposal__Proposal_Line_Item__c ProposalLineItem: Trigger.new){
        ...
        {
            myParentProposal.Has_Answer_Connect__c = true;
        }
        else
        {
            myParentProposal.Has_Answer_Connect__c = false ;
        }
    }
