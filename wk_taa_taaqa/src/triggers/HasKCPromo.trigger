trigger HasKCPromo on Quote_Promotion__c (after insert, after update, before delete) {
...
for (Quote_Promotion__c childObj : Trigger.old) {
    ...
    parentProposal = new Map<Id, Apttus_Proposal__Proposal__c>([SELECT id,Has_KC_Promotion__c,(Select id from Promotion_Groups__r where Promotion_Code__c = '24315') FROM Apttus_Proposal__Proposal__c WHERE ID IN :listIds]);
    ...
    for (Quote_Promotion__c QPromo: Trigger.old){
        ...
        {
            myParentProposal.Has_KC_Promotion__c = false;
        }
...
if(Trigger.IsInsert)
{
    ...
    for (Quote_Promotion__c QPromo: Trigger.new){
        ...
        if(parentProposal.containsKey(QPromo.Quote_Proposal__c) && parentProposal.get(QPromo.Quote_Proposal__c).Promotion_Groups__r.size() > 0)
        {
            myParentProposal.Has_KC_Promotion__c = true;
        }
