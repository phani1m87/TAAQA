trigger Has_ARM on Apttus_Proposal__Proposal_Line_Item__c (after insert, after update) {
 Map<ID, Apttus_Proposal__Proposal__c> parentProposal1 = new Map<ID, Apttus_Proposal__Proposal__c>();
    ...
    parentProposal1 = new Map<Id, Apttus_Proposal__Proposal__c>([SELECT id,Has_ARM__c,(Select id from R00N70000001yUfBEAU__r where Product_Sub_Family__c = 'Accounting Research Manager' LIMIT 1) FROM Apttus_Proposal__Proposal__c WHERE ID IN :listIds1]);
    ...
    for (Apttus_Proposal__Proposal_Line_Item__c ProposalLineItem1: Trigger.new){
        ...
        {
            myParentProposal1.Has_ARM__c = true;
        }
        else
        {
            myParentProposal1.Has_ARM__c = false ;
        }
    }
